...
impl MyApp {
    ...
    fn inventory_tab_ui(&mut self, ui: &mut egui::Ui) {
        ...
        egui::ScrollArea::both().show(ui, |ui| {
            egui::Grid::new("inventory_grid").striped(true).show(ui, |ui| {
                ui.strong("Name");
                ui.strong("SKU");
                ui.strong("Unit");
                ui.strong("Location");
    ...
    fn add_item_tab_ui(&mut self, ui: &mut egui::Ui) {
        ...
        egui::Grid::new("add_item_form").num_columns(2).show(ui, |ui| {
            ui.label("Name");
    ...
    fn movement_tab_ui(&mut self, ui: &mut egui::Ui, is_warehousing: bool) {
        ...
        egui::Grid::new("movement_form").num_columns(2).show(ui, |ui| {
            ui.label("Quantity");
            ui.text_edit_singleline(&mut form.quantity_text);
            ui.end_row();
            ...
            if let Some(item) = self.items.get_mut(form.item_index) {
                ...
                if new_qoh < 0 {
                    ...
                    self.transactions.push(Transaction {
                        ...
                        txn_type: if is_warehousing { TransactionType::Warehousing } else { TransactionType::Shipping },
                    });
        ...
        egui::ScrollArea::vertical().show(ui, |ui| {
            for txn in self.transactions.iter().rev().take(20) {
                let kind = match txn.txn_type { TransactionType::Warehousing => "IN", TransactionType::Shipping => "OUT" };
                let item_name = self.items.iter().find(|i| i.id == txn.item_id).map(|i| i.name.clone()).unwrap_or_else(|| "?".into());
